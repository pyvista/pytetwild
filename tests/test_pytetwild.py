...
@pytest.mark.parametrize(
    "mesh_generator", [pv.Icosphere, pv.examples.download_bunny_coarse]
)
...
def test_tetrahedralize(mesh_generator):
    ...
    assert isinstance(
        vertices_result, np.ndarray
    ), "The vertices result should be a numpy array"
    assert isinstance(
        tetrahedra_result, np.ndarray
    ), "The tetrahedra result should be a numpy array"
    assert (
        len(vertices_result) > 0
    ), "There should be more than 0 vertices in the result"
    assert (
        len(tetrahedra_result) > 0
    ), "There should be more than 0 tetrahedra in the result"
